...
fn main() {
    ...
    // The package name ends up baked into the binary, so it is always handed over explicitly:
    // `env!` reads rustc's environment, which cargo only fills from what this script asks for.
    // Announcing the variable as well is what makes a change to it rebuild the crate instead of
    // silently keeping the previous name.
    println!(
        "cargo:rustc-env=KSU_PACKAGE_NAME={}",
        env::var("KSU_PACKAGE_NAME").unwrap_or_else(|_| "me.weishu.kernelsu".to_string())
    );
