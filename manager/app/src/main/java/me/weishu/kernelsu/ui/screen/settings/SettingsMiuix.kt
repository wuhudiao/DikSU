...
import me.weishu.kernelsu.ui.util.WebUiLauncher
            ...
            LazyColumn(
                ...
                item {
                    ...
                    KsuIsValid {
                        ...
                        var launcherInstalled by remember { mutableStateOf(false) }
                        LaunchedEffect(Unit) {
                            withContext(Dispatchers.IO) {
                                launcherInstalled = WebUiLauncher.isInstalled(context)
                            }
                        }
                        Card(
                            modifier = Modifier
                            ...
                            SwitchPreference(
                                title = "桌面启动器",
                                summary = if (launcherInstalled) {
                                    "已安装。点桌面图标即启动网页端，关掉浏览器后自动退出（端口不监听）"
                                } else {
                                    "安装后可用桌面图标启动网页端，不用时端口不监听"
                                },
                                startAction = {
                                    Icon(
                                ...
                                checked = launcherInstalled,
                                onCheckedChange = { enabled ->
                                    scope.launch {
                                        val installed = withContext(Dispatchers.IO) {
                                            if (enabled) WebUiLauncher.install(context) else WebUiLauncher.uninstall(context)
                                            WebUiLauncher.isInstalled(context)
                                        }
                                        launcherInstalled = installed
                                        // Pin it to the home screen so there is an entry point
                                        // once the Manager itself is gone.
                                        if (enabled && installed) {
                                            WebUiLauncher.requestHomeShortcut(context)
                                        }
                                    }
                                }
                            )
