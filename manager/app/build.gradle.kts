...
val defaultManagerName = if (isPrBuild) "DikSU PR" else "DikSU"
...
// Package names of the embedded launcher variants, comma-separated. Supplied by
// ksu-build/build-all.sh; the fallback only matters for a plain local build.
val launcherPackages: List<String> =
    providers.gradleProperty("LAUNCHER_PACKAGE_NAMES").getOrElse("com.desk.tools")
        .split(',')
        .map { it.trim() }
        .filter { it.isNotEmpty() }

// Must match the number of <package> slots declared in AndroidManifest.xml.
val MAX_LAUNCHER_VARIANTS = 6
...
android {
    ...
    defaultConfig {
        ...
        // Package names of the embedded launcher variants, so the settings switch knows what
        // to install and uninstall.
        buildConfigField(
            "String[]",
            "LAUNCHER_PACKAGE_NAMES",
            launcherPackages.joinToString(", ", "new String[] {", "}") { "\"$it\"" },
        )
        // Declared as <queries> so the variants are visible to PackageManager despite
        // Android 11 package visibility. Manifest placeholders only substitute attribute
        // values, so the manifest lists one entry per slot rather than a generated block.
        for (i in 0 until MAX_LAUNCHER_VARIANTS) {
            manifestPlaceholders["launcher$i"] = launcherPackages.getOrElse(i) { launcherPackages.first() }
        }
...
dependencies {
    // Renaming and re-signing the launcher on the device, so each install can carry a package
    // name generated right there instead of one baked in at build time.
